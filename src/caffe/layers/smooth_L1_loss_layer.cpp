...
template <typename Dtype>
void SmoothL1Forward(const int n, const Dtype* in, Dtype* out, Dtype sigma2) {
...
template <typename Dtype>
void SmoothL1Backward(const int n, const Dtype* in, Dtype* out,
    Dtype sigma2) {
...
void SmoothL1LossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  SmoothL1Forward<Dtype>(count, diff_.cpu_data(), errors_.mutable_cpu_data(), sigma2_);
  if (has_weights_) {
  ...
  Dtype loss;
  loss = caffe_cpu_dot(count, ones_.cpu_data(), errors_.cpu_data());
...
void SmoothL1LossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  SmoothL1Backward<Dtype>(count, diff_.cpu_data(), diff_.mutable_cpu_data(), sigma2_); 
